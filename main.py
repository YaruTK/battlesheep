...
class Sheep:
    def __init__(self, head, tail, f: Field):
        ...
        if check_existence_possibility(self) and check_intersection(self) and check_size(self):
            for x in range(self.spawnbox[0][0], self.spawnbox[1][0] + 1):
                for y in range(self.spawnbox[0][1], self.spawnbox[1][1] + 1):
                    intersections[x][y] = 1
            add_ship(self, f)  # also triggers self.place(f)
            self.alive = True

    def place(self, field_class):
        for item in self.tiles:
            x, y = item
            field_class.matrix[x][y] += 2
...
def add_ship(s: Sheep, f: Field):
    dictionary_of_ships[s.length].append(s)
    s.place(f)
...
def announce_ships():  # there will be second announcer how many are still alive, might need separate class
...
if __name__ == '__main__':
    ...
    while count_ships(dictionary_of_ships) < sum(config.existence_of_the_ships.values()):
        ...
        shipx = Sheep(headx, tailx, field)  # adding new ship to dictionary
